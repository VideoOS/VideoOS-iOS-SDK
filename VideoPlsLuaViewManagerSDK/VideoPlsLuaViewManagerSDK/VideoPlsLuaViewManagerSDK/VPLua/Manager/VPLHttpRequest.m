...
static int lvNewHttpRequest(lua_State *L) {
    Class c = [LVUtil upvalueClass:L defaultClass:[VPLHttpRequest class]];
    {
        VPLHttpRequest* request = [[c alloc] init:L];
        {
...
static int cancel(lua_State *L) {
    ...
    if( user ){
        VPLHttpRequest* request = (__bridge VPLHttpRequest *)(user->object);
        if( [request isKindOfClass:[VPLHttpRequest class]] ){
            if(lua_gettop(L) >= 2) {
    ...
    if( user ){
        ...
        if( [request isKindOfClass:[VPLHttpRequest class]] ){
            ...
            if(argN >= 2) {
                ...
                __weak VPLHttpRequest *weakRequest = request;
        ...
        if( [request isKindOfClass:[VPLHttpRequest class]] ){
            ...
            if(argN >= 2) {
                ...
                api.apiRequestConstructingBodyBlock = ^(id<VPUPMultipartFormData> _Nonnull formData) {
                    ...
                    NSString *fileType = [VPLHttpRequest contentTypeForImageData:data];
