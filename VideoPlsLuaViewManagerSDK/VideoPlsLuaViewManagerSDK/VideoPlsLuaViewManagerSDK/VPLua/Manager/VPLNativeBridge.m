...
NSString *const VPLScreenChangeNotification = @"VPLScreenChangeNotification";
NSString *const VPLNotifyUserLoginedNotification = @"VPLNotifyUserLoginedNotification";
NSString *const VPLRequireLoginNotification = @"VPLRequireLoginNotification";
NSString *const VPLActionNotification = @"VPLActionNotification";
...
@interface VPLNativeBridge()

@end

@implementation VPLNativeBridge
...
+ (NSInteger)getVideoHeight:(lua_State *)l {
    ...
    if (luaNode.nodeController.isPortrait) {
        if (luaNode.nodeController.isFullScreen) {
...
+ (void)sendAction:(NSString*)action data:(id)data luaState:(lua_State *)l {
    VPLBaseNode *luaNode = [VPLNativeBridge luaNodeFromLuaState:l];
    [VPUPActionManager pushAction:action data:data sender:luaNode.nodeController];
}

+ (NSString *)cacheDataFilePath:(lua_State *)l {
    NSString *path = [VPUPPathUtil lOSPath];
    if ([path rangeOfString:@".bundle"].location != NSNotFound) {
        // 使用bundle加载lua,需要另外新建plist地址
        path = [[VPUPPathUtil lPath] stringByAppendingPathComponent:@"enjoy/cacheData.plist"];
    }
    else {
...
//        {"getAllCartData", getAllCartData},
        ...
        {"preloadLuaList", preloadLFileList},
...
static int appKey(lua_State *L) {
    lua_pushstring(L, [VPLSDK sharedSDK].appKey.UTF8String);
    return 1;
}

static int appSecret(lua_State *L) {
    lua_pushstring(L, [VPLSDK sharedSDK].appSecret.UTF8String);
    return 1;
}
...
static int nativeVideoID(lua_State *L) {
    NSString *nativeVideoID = [VPLSDK sharedSDK].videoInfo.nativeID;
    if(!nativeVideoID) {
        nativeVideoID = @"";
    }
...
static int getVideoSize(lua_State *L) {
    if (lua_gettop(L) >= 2) {
        ...
        switch (type) {
            ...
            default:
                w = [VPLNativeBridge getVideoWidth:L];
                h = [VPLNativeBridge getVideoHeight:L];
                originY = 0;
                break;
        }
        lua_pushnumber(L, w);
    ...
    {
        lua_pushnumber(L, [VPLNativeBridge getVideoWidth:L]);
        lua_pushnumber(L, [VPLNativeBridge getVideoHeight:L]);
        lua_pushnumber(L, 0);
    }
    return 3;
}
...
static int getVideoPosition(lua_State *L) {
    VPLBaseNode *luaNade = [VPLNativeBridge luaNodeFromLuaState:L];
    lua_pushnumber(L, luaNade.nodeController.rootView.bounds.origin.x);
    lua_pushnumber(L, luaNade.nodeController.rootView.bounds.origin.y);
    return 2;
}

static int isPortraitScreen(lua_State *L) {
    VPLBaseNode *luaNade = [VPLNativeBridge luaNodeFromLuaState:L];
    lua_pushboolean(L, luaNade.nodeController.isPortrait);
    return 1;
}

static int isFullScreen(lua_State *L) {
    VPLBaseNode *luaNade = [VPLNativeBridge luaNodeFromLuaState:L];
    lua_pushboolean(L, luaNade.nodeController.isFullScreen);
    return 1;
}
...
static int sendAction(lua_State *L) {
    if (lua_isstring(L, 2)) {
        ...
        [VPLNativeBridge sendAction:string data:dict luaState:L];
    }
    return 0;
}
...
static int cancel(lua_State *L) {
    if(lua_gettop(L) >= 2) {
        ...
        if (api) {
            ...
            NSDictionary *cacheData = [VPLNativeBridge getCacheData:L];
            if([cacheData objectForKey:apiId]) {
                NSString *value = [cacheData objectForKey:apiId];
                [LVUtil unregistry:L key:value];
            }
            [[VPLNetworkManager Manager].httpManager cancelAPIRequest:api];
        }
    }
    return 0;
}
...
static int cancelAll(lua_State *L) {
    [[VPLNetworkManager Manager].httpManager cancelAll];
    return 0;
}
...
//            [weakCell apiComplete:responseObject error:error response:response requestMethod:bRequestMethod];
        ...
        [VPLNativeBridge saveCacheData:[NSString stringWithFormat:@"%ld",api.apiId] value:bRequestMethod luaState:L];
        return 1;
    }
    return 0;
}
...
static int upload(lua_State *L) {
    ...
    if(argN >= 2) {
        ...
        api.apiRequestConstructingBodyBlock = ^(id<VPUPMultipartFormData> _Nonnull formData) {
            ...
            NSString *fileType = [VPLNativeBridge contentTypeForImageData:data];
...
static int updateLocalLuaFile(lua_State *L) {
    if( lua_gettop(L) >= 3) {
        ...
        if(lua && lua_md5) {
            [VPLNodeController saveLFileWithUrl:lua md5:lua_md5];
        }
    }
    return 0;
}
...
static int getSSID(lua_State *L) {
    NSString *ssid = [VPLSDK sharedSDK].videoInfo.ssid;
    if(!ssid) {
        ssid = @"";
    }
...
static int destroyView(lua_State *L) {
    if(lua_gettop(L) >= 2) {
        ...
        [VPLNativeBridge sendAction:VPUP_SchemeAddPath(nodeId,@"turnOff") data:@{@"id":nodeId} luaState:L];
    }
    else {
        VPLBaseNode *luaNode = [VPLNativeBridge luaNodeFromLuaState:L];
        [luaNode destroyView];
...
static int trackApi(lua_State *L) {
    ...
    if(argN >= 2) {
        ...
        VPLTrackApi *api = [[VPLTrackApi alloc] initWithTrackEventCat:cat params:data];
...
static int saveCacheData(lua_State *L) {
    ...
    if(argN >= 3) {
        ...
        BOOL success = [VPLNativeBridge saveCacheData:key value:value luaState:L];
    ...
    if( lua_gettop(L) >= 2) {
        if (lua_type(L, 2) == LUA_TTABLE) {
            ...
            if ([keys isKindOfClass:[NSArray class]] && [keys count] > 0) {
                ...
                NSMutableDictionary *changeData = [[NSMutableDictionary alloc] initWithDictionary:[VPLNativeBridge getCacheData:L]];
                ...
                if (hasChanged) {
                    [VPLNativeBridge saveAllCacheData:changeData luaState:L];
                }
            }
        }
    }
...
static int changeToPortrait(lua_State *L) {
    if( lua_gettop(L) >= 2 && lua_type(L, 2) == LUA_TBOOLEAN) {
        ...
        [VPLNativeBridge sendAction:VPUP_SchemeAddPath(@"toPortrait", @"device") data:dict luaState:L];
    }
    return 0;
}
...
static int widgetEvent(lua_State *L) {
    ...
    if (lua_gettop(L) >= 3) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:VPLActionNotification object:nil userInfo:dict];
    }
    return 0;
}
...
static int widgetNotify(lua_State *L) {
    ...
    if (lua_type(L, 2) == LUA_TTABLE) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:VPLActionNotification object:nil userInfo:dic];
    }
    return 0;
}
...
static int requireLogin(lua_State *L) {
    if (lua_gettop(L) >= 2 && lua_isfunction(L, 2)) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:VPLRequireLoginNotification object:nil userInfo:completeDict];
    }
    return 0;
}
...
static int notifyUserLogined(lua_State *L) {
    if (lua_gettop(L) >= 2 && lua_istable(L, 2)) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:VPLNotifyUserLoginedNotification object:nil userInfo:userInfo];
    }
    return 0;
}
...
static int currentDirection(lua_State *L) {
    VPLBaseNode *luaNode = [VPLNativeBridge luaNodeFromLuaState:L];
    lua_pushnumber(L, luaNode.nodeController.currentOrientation);
    return 1;
}
...
static int osType(lua_State *L) {
    lua_pushnumber(L, [VPLSDK sharedSDK].type);
    return 1;
}
    ...
    if (lua_gettop(L) >= 2) {
        if( lua_type(L, 2) == LUA_TTABLE ) {
            ...
            if ([array isKindOfClass:[NSArray class]] && array.count > 0) {
                ...
                VPUPPrefetchImageManager *prefetchImageManager = [[VPUPPrefetchImageManager alloc] initWithImageManager:[VPLNetworkManager Manager].imageManager];
...
static int preloadVideo(lua_State *L) {
    if (lua_gettop(L) >= 2) {
        if( lua_type(L, 2) == LUA_TTABLE ) {
            NSArray *array = lv_luaValueToNativeObject(L, 2);
            if ([array isKindOfClass:[NSArray class]] && array.count > 0) {
                [[VPLNetworkManager Manager].videoManager prefetchURLs:array complete:^(VPUPTrafficStatisticsList *trafficList) {
    ...
    if (lua_gettop(L) >= 2) {
        if( lua_type(L, 2) == LUA_TTABLE ) {
            ...
            if (appInfoObject != nil) {
                ...
                [[VPLDownloader sharedDownloader] checkAndDownloadFilesListWithAppInfo:appInfoObject complete:^(NSError * _Nonnull error, VPUPTrafficStatisticsList * _Nonnull trafficList) {
...
static int preloadLFileList(lua_State *L) {
    if (lua_gettop(L) >= 2) {
        if( lua_type(L, 2) == LUA_TTABLE ) {
            NSArray *array = lv_luaValueToNativeObject(L, 2);
            ...
            [[VPLDownloader sharedDownloader] checkAndDownloadFilesList:newArray complete:^(NSError * _Nonnull error, VPUPTrafficStatisticsList *trafficList) {
                ...
                NSLog(@"preloadLFileList error %@", error);
            }];
        }
...
static int videoOShost(lua_State *L) {
    lua_pushstring(L, [VPLServerHost UTF8String]);
    return 1;
}
