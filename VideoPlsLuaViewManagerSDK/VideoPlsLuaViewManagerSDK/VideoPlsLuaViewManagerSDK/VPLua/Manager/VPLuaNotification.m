...
@implementation VPLuaNotification
...
static int lvNewNotification(lua_State *L) {
    Class c = [LVUtil upvalueClass:L defaultClass:[VPLuaNotification class]];
    {
        VPLuaNotification* notification = [[c alloc] init:L];
        {
...
static int postNotification(lua_State *L) {
    ...
    if( user ){
        VPLuaNotification* notification = (__bridge VPLuaNotification *)(user->object);
        if( [notification isKindOfClass:[VPLuaNotification class]] ){
            if(lua_gettop(L) >= 2) {
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:VPLuaLuaDataNotification object:nil userInfo:@{key : data}];
        ...
        if( [notification isKindOfClass:[VPLuaNotification class]] ){
            if(lua_gettop(L) >= 3) {
                ...
                if (!notification.isRegisterNotification) {
                    ...
                    [[NSNotificationCenter defaultCenter] addObserver:notification selector:@selector(receiveNotification:) name:VPLuaLuaDataNotification object:nil];
                }
