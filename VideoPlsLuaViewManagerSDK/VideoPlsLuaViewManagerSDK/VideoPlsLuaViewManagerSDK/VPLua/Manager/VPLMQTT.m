...
NSString *const VPLMQTTClientMessageNotification = @"VPLMQTTClientMessageNotification";
...
@interface VPLMQTT () <VPUPMQTTObserverProtocol>

@property (nonatomic, copy) NSString *callBackMethod;

@end

@implementation VPLMQTT
...
static int lvNewMQTT (lua_State *L) {
    Class c = [LVUtil upvalueClass:L defaultClass:[VPLMQTT class]];
    {
        VPLMQTT* mqtt = [[c alloc] init:L];
...
static int mqttCallback(lua_State *L) {
    ...
    if (LVIsType(data, NativeObject) && lua_type(L, 2) == LUA_TFUNCTION) {
        VPLMQTT* luaMQTT = (__bridge VPLMQTT *)(data->object);
        if(luaMQTT) {
...
static int startMqtt(lua_State *L) {
    ...
    if( user ) {
        VPLMQTT* luaMQTT = (__bridge VPLMQTT *)(user->object);
        if(luaMQTT) {
            //2: MQTT topic qoc
            if( lua_gettop(L) >= 3) {
                ...
                if (lua_type(L, 2) == LUA_TTABLE) {
                    [[VPLCapacityManager sharedManager].messageTransferStation attachWithObserver:luaMQTT];
                    ...
                    if([topics isKindOfClass:[NSDictionary class]]) {
                        [topics enumerateKeysAndObjectsUsingBlock:^(NSString *topic, NSNumber *qos, BOOL * _Nonnull stop) {
                            [[VPLCapacityManager sharedManager].messageTransferStation addTopic:topic observer:luaMQTT];
                        }];
                    }
                }
            }
...
static int stopMqtt(lua_State *L) {
    ...
    if( user ) {
        VPLMQTT* luaMQTT = (__bridge VPLMQTT *)(user->object);
        if(luaMQTT) {
            [[VPLCapacityManager sharedManager].messageTransferStation detachWithObserver:luaMQTT];
        }
    }
    return 0;
}
