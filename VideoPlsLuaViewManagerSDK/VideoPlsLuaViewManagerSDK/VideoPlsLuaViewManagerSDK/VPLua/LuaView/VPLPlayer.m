...
NSString * const VPLPauseVideoPlayerNotification = @"VPLPauseVideoPlayerNotification";
#import "VPUPUrlUtil.h"

@interface VPLPlayer () <VPUPVideoClipProtocol>

@end

#define META_TABLE_MediaPlayer "UI.MediaPlayer"

@implementation VPLPlayer
...
- (id)init:(lua_State *)l {
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(pauseVideoPlayer:) name:VPLPauseVideoPlayerNotification object:nil];
    }
    return self;
}
...
#pragma -mark VPLPlayer
/*
 * lua脚本中 local player = Player() 对应的构造方法
 */
static int lvNewPlayer(lua_State *L) {
    // 获取构造方法对应的Class(Native类)
    Class c = [LVUtil upvalueClass:L defaultClass:[VPLPlayer class]];
    {
        VPLPlayer* player = [[c alloc] init:L];//通过Class和参数构造脚本对应的真实实例
        {
            ...
            luaL_getmetatable(L, META_TABLE_MediaPlayer ); // 获取VPLPlayer对应的类方法列表
...
static int source (lua_State *L) {
    ...
    if (user) {
        VPLPlayer *player = (__bridge VPLPlayer *)(user->object);// 获取self对应的native对象
        if( lua_gettop(L)>=2 ) {
            // 两个参数: 第一个对象自身, 第二个参数url
            NSString *url = lv_paramString(L, 2);// 2
            if( [player isKindOfClass:[VPLPlayer class]] ){
