...
@interface VPLTextField () <UITextFieldDelegate>

@property (nonatomic, assign) NSInteger maxLength;

@end

@implementation VPLTextField
...
static int lvNewTextField (lua_State *L) {
    Class c = [LVUtil upvalueClass:L defaultClass:[VPLTextField class]];
    
    VPLTextField* textFiled = [[c alloc] init:L];
    {
...
static int inputType (lua_State *L) {
    ...
    if( user ){
        VPLTextField* textField = (__bridge VPLTextField *)(user->object);
        if( [textField isKindOfClass:[VPLTextField class]] ){
            if (lua_gettop(L)>=2) {
...
static int textColor (lua_State *L) {
    ...
    if( user ){
        VPLTextField* view = (__bridge VPLTextField *)(user->object);// 获取userdata对应的native对象
        if( lua_gettop(L)>=2 ) {// 如果参数大于两个
            if( [view isKindOfClass:[VPLTextField class]] ){
...
static int textSize (lua_State *L) {
    ...
    if( user ){
        VPLTextField* view = (__bridge VPLTextField *)(user->object);// 获取self对应的native对象
        if( [view isKindOfClass:[VPLTextField class]] ){
            if( lua_gettop(L)>=2 ) {
...
{
    if (textField == self) {
        VPLTextField *luaTextField = (VPLTextField *)textField;
