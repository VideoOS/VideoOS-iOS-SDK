...
@interface VPLScrollView()

@end

#define META_TABLE_ScrollView "UI.VScrollView"

@implementation VPLScrollView
...
static int lvNewScrollView (lua_State *L) {
    Class c = [LVUtil upvalueClass:L defaultClass:[VPLScrollView class]];
    
    VPLScrollView* scrollView = [[c alloc] init:L];
    {
...
static int addSubview (lua_State *L) {
    ...
    if( father &&  LVIsType(son, View) ){
        VPLScrollView* superview = (__bridge VPLScrollView *)(father->object);
...
static int fullScroll (lua_State *L) {
    ...
    if (user) {
        VPLScrollView *scrollView = (__bridge VPLScrollView *)(user->object);// 获取self对应的native对象
        if( [scrollView isKindOfClass:[VPLScrollView class]] ){
