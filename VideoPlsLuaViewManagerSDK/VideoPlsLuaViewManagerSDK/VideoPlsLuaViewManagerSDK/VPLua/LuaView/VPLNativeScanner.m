...
@interface VPLNativeScanner() <VPUPImagePickerControllerDelegate>

@end

@implementation VPLNativeScanner
...
static int lvNewNativeScanner(lua_State *L) {
    Class c = [LVUtil upvalueClass:L defaultClass:[VPLNativeScanner class]];
    {
        VPLNativeScanner* native = [[c alloc] init:L];
        {
...
static int show(lua_State *L) {
    ...
    if( user ){
        VPLNativeScanner* native = (__bridge VPLNativeScanner *)(user->object);
        if (native) {
            if( lua_gettop(L) >= 2 ) {
