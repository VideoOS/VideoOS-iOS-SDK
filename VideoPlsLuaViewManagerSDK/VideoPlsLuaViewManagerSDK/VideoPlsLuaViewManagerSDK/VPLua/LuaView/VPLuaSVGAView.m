...
#pragma -mark VPLuaSVGAView
/*
 * lua脚本中 local svgaView = SVGAView() 对应的构造方法
 */
static int lvNewSVGAView(lua_State *L) {
    // 获取构造方法对应的Class(Native类)
    Class c = [LVUtil upvalueClass:L defaultClass:[VPLuaSVGAView class]];
    {
        VPLuaSVGAView* svgaView = [[c alloc] init:L];//通过Class和参数构造脚本对应的真实实例
        {
...
static int loops (lua_State *L) {
    ...
    if( user ){
        VPLuaSVGAView* view = (__bridge VPLuaSVGAView *)(user->object);// 获取self对应的native对象
        if( lua_gettop(L)>=2 ) {
            // 两个参数: 第一个对象自身, 第二个参数loops
            int loops = (int)lua_tonumber(L, 2);// 2
            if( [view isKindOfClass:[VPLuaSVGAView class]] ){
...
+(int) lvClassDefine:(lua_State *)L globalName:(NSString*) globalName{
    // 注册构造方法: "svgaView" 对应的C函数(lvNewSVGAView) + 对应的类Class(self/VPLuaSVGAView)
    ...
    const struct luaL_Reg memberFunctions [] = {
        ...
        { callbackSVGAViewKeys[kVPLuaSVGAViewCallbackOnFinished], onFinished },
        { callbackSVGAViewKeys[kVPLuaSVGAViewCallbackOnStep], onStep },
        {NULL, NULL}
    };
...
- (void)callback:(VPLuaSVGAViewCallback)idx {
...
- (void)callback:(VPLuaSVGAViewCallback)idx progress:(NSInteger)progress {
    lua_State* l = self.lv_luaviewCore.l;
    if (l && self.lv_userData) {
        int stackIndex = lua_gettop(l);
