...
@implementation VPLGradientView
...
//        [self.layer insertSublayer:self.gradientLayer atIndex:0];
    ...
    if (self.cornerLayer) {
        ...
        UIBezierPath *path = [VPLGradientView maskCornerRadiiValue:self.radiiValue rect:self.bounds];
    ...
    if (self.pathLayer) {
        ...
        if (self.cornerLayer) {
            path = [VPLGradientView maskCornerRadiiValue:self.radiiValue rect:self.bounds];
        }
        else {
...
static int lvNewGradientView (lua_State *L){
    Class c = [LVUtil upvalueClass:L defaultClass:[VPLGradientView class]];
    {
        VPLGradientView* gradientView = [[c alloc] init:L];
        {
...
static int gradient(lua_State *L) {
    ...
    if(user) {
        VPLGradientView* gradientView = (__bridge VPLGradientView *)(user->object);
...
static int corner(lua_State *L) {
    ...
    if(user) {
        ...
        if (lua_gettop(L) >= 9) {
            ...
            UIBezierPath *path = [VPLGradientView maskCornerRadiiValue:positions rect:gradientView.bounds];
            maskLayer.path = path.CGPath;
    ...
    if(user) {
        ...
        if (lua_gettop(L) >= 3) {
            ...
            if (gradientView.cornerLayer) {
                path = [VPLGradientView maskCornerRadiiValue:gradientView.radiiValue rect:gradientView.bounds];
            }
            else {
