...
-(void) setImageByName:(NSString*) imageName{
    ...
    if( [LVUtil isExternalUrl:imageName] ){
        //CDN image
        __weak VPLImageView* weakImageView = self;
...
static int lvNewImageView(lua_State *L) {
    Class c = [LVUtil upvalueClass:L defaultClass:[VPLImageView class]];
    
    NSString* imageName = lv_paramString(L, 1);
    
    VPLImageView* imageView = [[c alloc] init:L];
...
static int setImage (lua_State *L) {
    ...
    if( user ){
        VPLImageView* imageView = (__bridge VPLImageView *)(user->object);
        if ( [imageView isKindOfClass:[VPLImageView class]] ) {
