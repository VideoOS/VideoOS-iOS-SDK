...
@class VPUPAlbumModel,VPUPAssetModel;
@protocol VPUPImagePickerControllerDelegate;
@interface VPUPImagePickerManager : NSObject
...
@property (assign, nonatomic) id<VPUPImagePickerControllerDelegate> pickerDelegate;
...
/// Get Album 获得相册/相册数组
- (void)getCameraRollAlbum:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(VPUPAlbumModel *model))completion;
- (void)getAllAlbums:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSArray<VPUPAlbumModel *> *models))completion;

/// Get Assets 获得Asset数组
- (void)getAssetsFromFetchResult:(id)result allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSArray<VPUPAssetModel *> *models))completion;
- (void)getAssetFromFetchResult:(id)result atIndex:(NSInteger)index allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(VPUPAssetModel *model))completion;

/// Get photo 获得照片
- (void)getPostImageWithAlbumModel:(VPUPAlbumModel *)model completion:(void (^)(UIImage *postImage))completion;
...
/// 获取asset的资源类型
- (VPUPAssetModelMediaType)getAssetType:(id)asset;
