...
//based on LFImagePickerController
//https://github.com/lincf0912/LFImagePickerController

#import <UIKit/UIKit.h>
//#import "NSBundle+VPUPImagePicker.h"
...
@protocol VPUPImagePickerControllerDelegate;
@interface VPUPImagePickerController : UINavigationController
...
///pushPhotoPickerVc为YES时显示VPUPPhotoPickerController，为NO时显示VPUPAlbumPickerController
...
/// Default is NO, if set NO, user can't picking video.
/// 默认为NO，如果设置为NO,用户将不能选择视频
...
/// Default is NO, if set NO, user can't take picture.
/// 默认为NO，如果设置为NO,拍照按钮将隐藏,用户将不能选择照片
...
@property (nonatomic, strong) NSMutableArray<VPUPAssetModel *> *selectedModels;
...
// You can get original photo with asset, by the method [[VPUPImageManager manager] getOriginalPhotoWithAsset:completion:].
// The UIImage Object in photos default width is 828px, you can set it by photoWidth property.
// 这个照片选择器会自己dismiss，当选择器dismiss的时候，会执行下面的handle
// 你也可以设置autoDismiss属性为NO，选择器就不会自己dismis了
// 如果isSelectOriginalPhoto为YES，表明用户选择了原图
// 你可以通过一个asset获得原图，通过这个方法：[[VPUPImageManager manager] getOriginalPhotoWithAsset:completion:]
...
@property (nonatomic, copy) void (^didFinishPickingPhotosWithFilePathHandle)(NSString*);
...
@property (nonatomic, weak) id<VPUPImagePickerControllerDelegate> pickerDelegate;

@end


@protocol VPUPImagePickerControllerDelegate <NSObject>
@optional
// The picker should dismiss itself; when it dismissed these handle will be called.
// You can also set autoDismiss to NO, then the picker don't dismiss itself.
// If isOriginalPhoto is YES, user picked the original photo.
...
//- (void)imagePickerController:(VPUPImagePickerController *)picker didFinishPickingPhotos:(NSArray<UIImage *> *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto;
//- (void)imagePickerController:(VPUPImagePickerController *)picker didFinishPickingPhotos:(NSArray<UIImage *> *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto infos:(NSArray<NSDictionary *> *)infos;
//- (void)imagePickerControllerDidCancel:(VPUPImagePickerController *)picker __attribute__((deprecated("Use -VPUP_imagePickerControllerDidCancel:.")));
...
// 如果用户选择了一个视频，下面的handle会被执行
// 如果系统版本大于iOS8，asset是PHAsset类的对象，否则是ALAsset类的对象
//- (void)imagePickerController:(VPUPImagePickerController *)picker didFinishPickingVideo:(UIImage *)coverImage sourceAssets:(id)asset;

// If user picking a gif image, this callback will be called.
// 如果用户选择了一个gif图片，下面的handle会被执行
//- (void)imagePickerController:(VPUPImagePickerController *)picker didFinishPickingGifImage:(UIImage *)animatedImage sourceAssets:(id)asset;
...
@interface VPUPAlbumPickerController : UIViewController
...
@interface VPUPCommonTools : NSObject
+ (BOOL)vpup_isIPhoneX;
