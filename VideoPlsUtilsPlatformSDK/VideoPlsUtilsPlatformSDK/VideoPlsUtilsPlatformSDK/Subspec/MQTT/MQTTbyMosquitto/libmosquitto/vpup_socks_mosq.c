...
int vpup_mosquitto_socks5_set(struct vpup_mosquitto *mosq, const char *host, int port, const char *username, const char *password)
{
#ifdef WITH_SOCKS
	...
	if(mosq->socks5_host){
		_vpup_mosquitto_free(mosq->socks5_host);
	}

	mosq->socks5_host = _vpup_mosquitto_strdup(host);
	if(!mosq->socks5_host){
	...
	if(mosq->socks5_username){
		_vpup_mosquitto_free(mosq->socks5_username);
	}
	if(mosq->socks5_password){
		_vpup_mosquitto_free(mosq->socks5_password);
	}

	if(username){
		mosq->socks5_username = _vpup_mosquitto_strdup(username);
		if(!mosq->socks5_username){
			return MOSQ_ERR_NOMEM;
		}

		if(password){
			mosq->socks5_password = _vpup_mosquitto_strdup(password);
			if(!mosq->socks5_password){
				_vpup_mosquitto_free(mosq->socks5_username);
				return MOSQ_ERR_NOMEM;
			}
...
int vpup_mosquitto__socks5_send(struct vpup_mosquitto *mosq)
{
	...
	if(mosq->state == mosq_cs_socks5_new){
		...
		packet->payload = _vpup_mosquitto_malloc(sizeof(uint8_t)*packet->packet_length);
		...
		mosq->in_packet.payload = _vpup_mosquitto_malloc(sizeof(uint8_t)*2);
		if(!mosq->in_packet.payload){
			_vpup_mosquitto_free(packet->payload);
		...
		mosq->in_packet.payload = _vpup_mosquitto_malloc(sizeof(uint8_t)*5);
		if(!mosq->in_packet.payload){
			_vpup_mosquitto_free(packet->payload);
			_vpup_mosquitto_free(packet);
			return MOSQ_ERR_NOMEM;
...
int vpup_mosquitto__socks5_read(struct vpup_mosquitto *mosq)
{
	...
	if(mosq->state == mosq_cs_socks5_start){
		while(mosq->in_packet.to_process > 0){
			len = _vpup_mosquitto_net_read(mosq, &(mosq->in_packet.payload[mosq->in_packet.pos]), mosq->in_packet.to_process);
			if(len > 0){
		...
		switch(mosq->in_packet.payload[1]){
			case SOCKS_AUTH_NONE:
				...
				return vpup_mosquitto__socks5_send(mosq);
		...
		if(mosq->in_packet.packet_length == 5){
			...
			payload = _vpup_mosquitto_realloc(mosq->in_packet.payload, mosq->in_packet.packet_length);
			if(payload){
		...
		return _vpup_mosquitto_packet_read(mosq);
	}
	return MOSQ_ERR_SUCCESS;
}
#endif
