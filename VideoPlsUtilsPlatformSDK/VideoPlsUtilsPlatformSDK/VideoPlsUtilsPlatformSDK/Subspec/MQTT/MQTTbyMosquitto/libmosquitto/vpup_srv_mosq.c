...
{   
	struct vpup_mosquitto *mosq = arg;
	...
	if(status == ARES_SUCCESS){
		...
		_vpup_mosquitto_log_printf(mosq, VPUP_MOSQ_LOG_ERR, "Error: SRV lookup failed (%d).", status);
...
int vpup_mosquitto_connect_srv(struct vpup_mosquitto *mosq, const char *host, int keepalive, const char *bind_address)
{
#ifdef WITH_SRV
	char *h;
	int rc;
	...
	if(!host){
		...
		if(mosq->tls_cafile || mosq->tls_capath || mosq->tls_psk){
			h = _vpup_mosquitto_malloc(strlen(host) + strlen("_secure-mqtt._tcp.") + 1);
			if(!h) return MOSQ_ERR_NOMEM;
			sprintf(h, "_secure-mqtt._tcp.%s", host);
		}else{
#endif
			h = _vpup_mosquitto_malloc(strlen(host) + strlen("_mqtt._tcp.") + 1);
			if(!h) return MOSQ_ERR_NOMEM;
