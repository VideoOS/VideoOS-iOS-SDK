...
@implementation VPUPSDWebImageWebPCoder {
    WebPIDecoder *_idec;
}
...
+ (instancetype)sharedCoder {
    static VPUPSDWebImageWebPCoder *coder;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        coder = [[VPUPSDWebImageWebPCoder alloc] init];
    });
    return coder;
}
...
- (BOOL)canDecodeFromData:(nullable NSData *)data {
    return ([NSData vpupsd_imageFormatForImageData:data] == VPUPSDImageFormatWebP);
}
...
- (UIImage *)decodedImageWithData:(NSData *)data {
    ...
    CGContextRef canvas = CGBitmapContextCreate(NULL, canvasWidth, canvasHeight, 8, 0, VPUPSDCGColorSpaceGetDeviceRGB(), bitmapInfo);
    if (!canvas) {
    ...
    if (!(flags & ANIMATION_FLAG)) {
        ...
        UIImage *staticImage = [self vpupsd_rawWebpImageWithData:webpData];
        if (staticImage) {
    ...
    do {
        @autoreleasepool {
            UIImage *image = [self vpupsd_drawnWebpImageWithCanvas:canvas iterator:iter];
            if (!image) {
                continue;
            }
            ...
            VPUPSDWebImageFrame *frame = [VPUPSDWebImageFrame frameWithImage:image duration:duration / 1000.f];
    ...
    UIImage *animatedImage = [VPUPSDWebImageCoderHelper animatedImageWithFrames:frames];
...
- (UIImage *)incrementallyDecodedImageWithData:(NSData *)data finished:(BOOL)finished {
    ...
    if (width + height > 0 && last_y > 0 && height >= last_y) {
        ...
        CGColorSpaceRef colorSpaceRef = VPUPSDCGColorSpaceGetDeviceRGB();
        ...
        CGContextRef canvas = CGBitmapContextCreate(NULL, width, height, 8, 0, VPUPSDCGColorSpaceGetDeviceRGB(), bitmapInfo);
        if (!canvas) {
...
- (nullable UIImage *)vpupsd_drawnWebpImageWithCanvas:(CGContextRef)canvas iterator:(WebPIterator)iter {
    UIImage *image = [self vpupsd_rawWebpImageWithData:iter.fragment];
    if (!image) {
        return nil;
    }
...
- (nullable UIImage *)vpupsd_rawWebpImageWithData:(WebPData)webpData {
...
- (BOOL)canEncodeToFormat:(VPUPSDImageFormat)format {
    return (format == VPUPSDImageFormatWebP);
}
...
- (NSData *)encodedDataWithImage:(UIImage *)image format:(VPUPSDImageFormat)format {
    ...
    if (frames.count == 0) {
        // for static single webp image
        data = [self vpupsd_encodedWebpDataWithImage:image];
    } else {
        // for animated webp image
        ...
        for (size_t i = 0; i < frames.count; i++) {
            VPUPSDWebImageFrame *currentFrame = frames[i];
            NSData *webpData = [self vpupsd_encodedWebpDataWithImage:currentFrame.image];
        ...
        int loopCount = (int)image.vpupsd_imageLoopCount;
...
- (nullable NSData *)vpupsd_encodedWebpDataWithImage:(nullable UIImage *)image {
    if (!image) {
        return nil;
    }
