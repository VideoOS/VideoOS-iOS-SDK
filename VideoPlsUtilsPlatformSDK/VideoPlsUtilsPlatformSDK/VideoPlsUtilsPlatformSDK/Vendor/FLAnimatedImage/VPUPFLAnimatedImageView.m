...
@protocol VPUPFLAnimatedImageViewDebugDelegate <NSObject>
@optional
- (void)debug_animatedImageView:(VPUPFLAnimatedImageView *)animatedImageView waitingForFrame:(NSUInteger)index duration:(NSTimeInterval)duration;
@end
#endif


@interface VPUPFLAnimatedImageView ()
...
#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<VPUPFLAnimatedImageViewDebugDelegate> debug_delegate;
#endif

@end


@implementation VPUPFLAnimatedImageView
...
- (void)setAnimatedImage:(VPUPFLAnimatedImage *)animatedImage
{
...
- (NSTimeInterval)frameDelayGreatestCommonDivisor
{
    ...
    const NSTimeInterval kGreatestCommonDivisorPrecision = 2.0 / kVPUPFLAnimatedImageDelayTimeIntervalMinimum;
...
- (void)startAnimating
{
    if (self.animatedImage) {
        ...
        if (!self.displayLink) {
            ...
            VPUPFLWeakProxy *weakProxy = [VPUPFLWeakProxy weakProxyForObject:self];
