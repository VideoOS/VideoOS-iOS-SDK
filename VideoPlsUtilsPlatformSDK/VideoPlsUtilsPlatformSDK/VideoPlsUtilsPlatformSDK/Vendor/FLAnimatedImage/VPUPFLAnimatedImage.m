...
const NSTimeInterval kVPUPFLAnimatedImageDelayTimeIntervalMinimum = 0.02;
...
typedef NS_ENUM(NSUInteger, VPUPFLAnimatedImageDataSizeCategory) {
    VPUPFLAnimatedImageDataSizeCategoryAll = 10,       // All frames permanently in memory (be nice to the CPU)
    VPUPFLAnimatedImageDataSizeCategoryDefault = 75,   // A frame cache of default size in memory (usually real-time performance and keeping low memory profile)
    VPUPFLAnimatedImageDataSizeCategoryOnDemand = 250, // Only keep one frame at the time in memory (easier on memory, slowest performance)
    VPUPFLAnimatedImageDataSizeCategoryUnsupported     // Even for one frame too large, computer says no.
};

typedef NS_ENUM(NSUInteger, VPUPFLAnimatedImageFrameCacheSize) {
    VPUPFLAnimatedImageFrameCacheSizeNoLimit = 0,                // 0 means no specific limit
    VPUPFLAnimatedImageFrameCacheSizeLowMemory = 1,              // The minimum frame cache size; this will produce frames on-demand.
    VPUPFLAnimatedImageFrameCacheSizeGrowAfterMemoryWarning = 2, // If we can produce the frames faster than we consume, one frame ahead will already result in a stutter-free playback.
    VPUPFLAnimatedImageFrameCacheSizeDefault = 5                 // Build up a comfy buffer window to cope with CPU hiccups etc.
};


#if defined(DEBUG) && DEBUG
@protocol VPUPFLAnimatedImageDebugDelegate <NSObject>
@optional
- (void)debug_animatedImage:(VPUPFLAnimatedImage *)animatedImage didUpdateCachedFrames:(NSIndexSet *)indexesOfFramesInCache;
- (void)debug_animatedImage:(VPUPFLAnimatedImage *)animatedImage didRequestCachedFrame:(NSUInteger)index;
- (CGFloat)debug_animatedImagePredrawingSlowdownFactor:(VPUPFLAnimatedImage *)animatedImage;
@end
#endif


@interface VPUPFLAnimatedImage ()
...
#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<VPUPFLAnimatedImageDebugDelegate> debug_delegate;
#endif
...
@implementation VPUPFLAnimatedImage
...
- (NSUInteger)frameCacheSizeCurrent
{
    ...
    if (self.frameCacheSizeMax > VPUPFLAnimatedImageFrameCacheSizeNoLimit) {
        frameCacheSizeCurrent = MIN(frameCacheSizeCurrent, self.frameCacheSizeMax);
    }
    
    if (self.frameCacheSizeMaxInternal > VPUPFLAnimatedImageFrameCacheSizeNoLimit) {
...
+ (void)initialize
{
    if (self == [VPUPFLAnimatedImage class]) {
...
- (instancetype)init
{
    VPUPFLAnimatedImage *animatedImage = [self initWithAnimatedGIFData:nil];
    if (!animatedImage) {
...
//                                FLLog(FLLogLevelInfo, @"Falling back to preceding delay time for frame %zu %@ because none found in GIF properties %@", i, frameImage, frameProperties);
                        ...
                        if ([delayTime floatValue] < ((float)kVPUPFLAnimatedImageDelayTimeIntervalMinimum - FLT_EPSILON)) {
//                            FLLog(FLLogLevelInfo, @"Rounding frame %zu's `delayTime` from %f up to default %f (minimum supported: %f).", i, [delayTime floatValue], kDelayTimeIntervalDefault, kVPUPFLAnimatedImageDelayTimeIntervalMinimum);
                            delayTime = @(kDelayTimeIntervalDefault);
                        }
...
//            FLLog(FLLogLevelInfo, @"Created valid GIF but with only a single frame. Image properties: %@", imageProperties);
        ...
        if (optimalFrameCacheSize == 0) {
            ...
            if (animatedImageDataSize <= VPUPFLAnimatedImageDataSizeCategoryAll) {
                _frameCacheSizeOptimal = self.frameCount;
            } else if (animatedImageDataSize <= VPUPFLAnimatedImageDataSizeCategoryDefault) {
                ...
                _frameCacheSizeOptimal = VPUPFLAnimatedImageFrameCacheSizeDefault;
            } else {
                // The predicted size exceeds the limits to build up a cache and we go into low memory mode from the beginning.
                _frameCacheSizeOptimal = VPUPFLAnimatedImageFrameCacheSizeLowMemory;
            }
        } else {
...
{
    VPUPFLAnimatedImage *animatedImage = [[VPUPFLAnimatedImage alloc] initWithAnimatedGIFData:data];
    return animatedImage;
}
...
//        FLLog(FLLogLevelWarn, @"Two-part frame cache range doesn't equal full range.");
    ...
    VPUPFLAnimatedImage * __weak weakSelf = self;
...
+ (CGSize)sizeForImage:(id)image
{
    ...
    } else if ([image isKindOfClass:[VPUPFLAnimatedImage class]]) {
        VPUPFLAnimatedImage *animatedImage = (VPUPFLAnimatedImage *)image;
...
{
    self.frameCacheSizeMaxInternal = VPUPFLAnimatedImageFrameCacheSizeNoLimit;
...
- (void)didReceiveMemoryWarning:(NSNotification *)notification
{
    ...
    [NSObject cancelPreviousPerformRequestsWithTarget:self.weakProxy selector:@selector(growFrameCacheSizeAfterMemoryWarning:) object:@(VPUPFLAnimatedImageFrameCacheSizeGrowAfterMemoryWarning)];
...
//    FLLog(FLLogLevelDebug, @"Attempt setting frame cache size max to %lu (previous was %lu) after memory warning #%lu for animated image: %@", (unsigned long)VPUPFLAnimatedImageFrameCacheSizeLowMemory, (unsigned long)self.frameCacheSizeMaxInternal, (unsigned long)self.memoryWarningCount, self);
    self.frameCacheSizeMaxInternal = VPUPFLAnimatedImageFrameCacheSizeLowMemory;
    ...
    if ((self.memoryWarningCount - 1) <= kGrowAttemptsMax) {
        [self.weakProxy performSelector:@selector(growFrameCacheSizeAfterMemoryWarning:) withObject:@(VPUPFLAnimatedImageFrameCacheSizeGrowAfterMemoryWarning) afterDelay:kGrowDelay];
    }
...
@implementation VPUPFLWeakProxy
...
{
    VPUPFLWeakProxy *weakProxy = [VPUPFLWeakProxy alloc];
